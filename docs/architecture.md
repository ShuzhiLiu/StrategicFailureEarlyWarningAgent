...
**Per-sentence (L2.3-4)** — `validate_sentence_citations()` splits each factor's `claim` + `description` into sentences, then runs **two matchers in series**: (1) **token-overlap (primary, iter 44)** — content-token coverage with stopword filtering; requires ≥3 distinct hit tokens AND ≥0.55 ratio against the sentence's content tokens; locates the tightest evidence window via word-boundary scan. (2) **`difflib` longest-block (fallback, iter 43)** — looks for the longest contiguous matching block (≥25% of sentence length, ≥12 chars). Matched sentences record a `(doc_id, char_start, char_end)` span; unresolved sentences land in `audit_violations.sentence_citations_unresolved`. This is *soft enforcement* — both paths are fuzzy by design (analysts paraphrase rather than quote verbatim) and data is logged for the audit trail rather than raising. **English content paraphrases now resolve cleanly** — the iter-44 token-overlap path lifts paraphrase recall to >66% on English↔English smoke corpora. Production resolution rates on retrospectives that mix English claims with non-English filing text (Honda/Toyota EDINET, BYD CNINFO) are lower; tightening cross-lingual matching is on the next-iteration roadmap (character-bigram tokens for CJK content, or asking the analyst LLM to emit explicit sentence→evidence_id maps as structured output). Either way the audit trail is honest: every sentence is recorded with its resolution status, so reviewers see exactly which conclusions trace cleanly to source spans.
...
- **Sentence-level matcher is fuzzy.** Iter 44 added a token-overlap primary path that lifts paraphrase recall on English content (smoke corpora >66%). The `difflib` longest-block fallback still handles verbatim quotes and CJK. Tightening cross-lingual matching (English claims paired with non-English filing text) is on the next-iteration roadmap — most likely via character-bigram tokens, embedding similarity, or asking the analyst LLM to emit explicit sentence→evidence_id maps as structured output.
